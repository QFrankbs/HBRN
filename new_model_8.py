...
class TSCA(nn.Module):
    def __init__(self, args):
        super(TSCA, self).__init__()

        self.msrt1 = MSRT(args)
        self.msrt2 = MSRT(args)
        self.msrt3 = MSRT(args)
        #self.msrt4 = MSRT(args)
        self.slen = args.lang_seq_len
        self.alen = args.audio_seq_len
        self.vlen = args.video_seq_len
        self.retnet_rel_pos1 = RetNetRelPos(args)
        self.retnet_rel_pos2 = RetNetRelPos(args)
        self.retnet_rel_pos3 = RetNetRelPos(args)
        #self.retnet_rel_pos4 = RetNetRelPos(args)
        ...
        #self.norm4 = LayerNorm(args.hidden_size)
        
        self.dropout5 = nn.Dropout(args.dropout_r)
        self.norm5 = LayerNorm(args.hidden_size)
    ...
    def forward(self, x, y, z):
        ...
        x = self.norm3(x + self.dropout3(
            self.msrt3(x, x, x, x, self.retnet_rel_pos3(self.slen))
        ))
        x = self.norm2(x + self.dropout2(
            self.msrt2(xyz, x, x, xyz, self.retnet_rel_pos2(self.alen))
        ))
        x = self.norm1(x + self.dropout1(
            self.msrt1(x, x, x, x, self.retnet_rel_pos1(self.slen))
        ))
...
class ASCA(nn.Module):
    ...
    def forward(self, x, y):
        ...
        xy = x * y
        x = self.norm3(x + self.dropout3(
            self.msrt3(x, x, x, x, self.retnet_rel_pos3(self.alen))
        ))
        x = self.norm2(x + self.dropout2(
            self.msrt2(xy, x, x, xy, self.retnet_rel_pos2(self.vlen))
        ))
        x = self.norm1(x + self.dropout1(
            self.msrt1(x, x, x, x, self.retnet_rel_pos1(self.alen))
        ))
        # x = self.norm3(x + self.dropout3(
        #     self.msrt3(z, x, x, z, self.retnet_rel_pos3(self.vlen))
        # ))
        #x = self.norm4(x + self.dropout4(
        #    self.msrt4(x, x, x, x, self.retnet_rel_pos4(self.slen))
        #))
        x = self.norm5(x + self.dropout5(
            self.ffn(x)
        ))
...
class VSCA(nn.Module):
    def __init__(self, args):
        super(VSCA, self).__init__()
    ...
    def forward(self, x):
        x = self.norm1(x + self.dropout1(
            self.msrt1(x, x, x, x, self.retnet_rel_pos1(self.vlen))
        ))
        x = self.norm2(x + self.dropout2(
            self.msrt2(x, x, x, x, self.retnet_rel_pos2(self.vlen))
        ))
        x = self.norm3(x + self.dropout3(
            self.msrt3(x, x, x, x, self.retnet_rel_pos3(self.vlen))
        ))
        # x = self.norm2(x + self.dropout2(
        #     self.msrt2(y, x, x, y, self.retnet_rel_pos2(self.vlen))
        # ))
        # x = self.norm3(x + self.dropout3(
        #     self.msrt3(z, x, x, z, self.retnet_rel_pos3(self.vlen))
        # ))
...
class Block(nn.Module):
    ...
    def forward(self, x, x_mask, y, y_mask, z, z_mask):
        ...
        ax1 = self.sa1(x,x_mask)
        ay1 = self.sa2(y,y_mask)
        az1 = self.sa3(z,z_mask)
        ...
        return self.norm_l(x + self.dropout(ax)+self.dropout(ax1)), \
               self.norm_a(y + self.dropout(ay)+self.dropout(ay1)), \
               self.norm_v(z + self.dropout(az)+self.dropout(az1))
...
class Model_LAV_RETNET(nn.Module):
    ...
    def forward(self, x, y, z, x1):
        ...
        cl_loss = self.InfoNCE(x*y*z,x,x1)
